...
DRIVE_FILE_ID = "1csR51feB60Uvzh3Qp4iMYuV9C0EkSMy_"  
...
CLASS_REPORTS = {
    "Benign – Other": (
        "This category includes harmless, non-cancerous skin lesions such as "
        "seborrheic keratoses or benign keratoses. These typically do not require "
        "treatment, but monitoring for changes is advised."
    ),

    "Cancer / Pre-cancerous": (
        "This category indicates a lesion that may be cancerous or at risk of becoming cancer. "
        "Examples include actinic keratosis or early carcinomas. A clinical evaluation is recommended."
    ),

    "Melanocytic Nevus : Usually benign": (
        "A melanocytic nevus (mole) is typically benign and common. "
        "Monitor for changes in shape, color, or size using the ABCDE rule."
    ),

    "Melanoma : Cancer (malignant)": (
        "Melanoma is a serious and potentially aggressive form of skin cancer. "
        "Immediate clinical examination by a dermatologist is strongly recommended."
    )
}
...
# Streamlit UI
# -------------------------
st.title("Skin_Cancer Early check")
